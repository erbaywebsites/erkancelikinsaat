...
export default function FeaturesSection({ projectData }: FeaturesSectionProps) {
          ...
          {projectData.features.map((f, i) => (
            ...
            >
              <div className="text-gold mb-5 group-hover:scale-110 transition-transform duration-300">
                {iconMap[f.iconName] || iconMap.quality}
              </div>
          ...
          {projectData.stats.map((s) => (
            <div key={s.label} className="bg-card py-10 px-6 text-center">
              <p className="font-serif text-4xl sm:text-5xl font-light text-gold">
                {s.val}
                <span className="text-2xl text-gold-dim">{s.unit}</span>
              </p>
